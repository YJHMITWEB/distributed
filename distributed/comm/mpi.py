...
TAG_QUOTA_PER_CONNECTION = 150000
...
class MPI4Dask(Comm):
    ...
    async def mpi_send_large(self, buf, size, _tag):
        ...
        for i in range(num_of_blks):
            ...
            if type(buf) == rmm._lib.device_buffer.DeviceBuffer:
                ...
                r = MPI.COMM_WORLD.Isend([buf[s_idx:e_idx], MPI.BYTE], dest=you, tag=_tag)
            _tag = _tag + 1
            reqs.append(r)
        ...
        if last_blk_size is not 0:
            ...
            if type(buf) == rmm._lib.device_buffer.DeviceBuffer:
                ...
                r = MPI.COMM_WORLD.Isend([buf[s_idx:e_idx], MPI.BYTE], \
                                                           dest=you, tag=_tag)
    ...
    async def mpi_send(self, buf, size, _tag):
        ...
        req = MPI.COMM_WORLD.Isend([buf, MPI.BYTE], dest=you, tag=_tag)
    ...
    async def mpi_recv_large(self, buf, size, _tag):
        ...
        for i in range(num_of_blks):
            ...
            if type(buf) == rmm._lib.device_buffer.DeviceBuffer:
                ...
                r = MPI.COMM_WORLD.Irecv([buf[s_idx:e_idx], MPI.BYTE], source=you, \
                                                                   tag=_tag)
            _tag = _tag + 1
            reqs.append(r)
        ...
        if last_blk_size is not 0:
            ...
            if type(buf) == rmm._lib.device_buffer.DeviceBuffer:
                ...
                r = MPI.COMM_WORLD.Irecv([buf[s_idx:e_idx], MPI.BYTE], \
                                                    source=you, tag=_tag)
            _tag = _tag + 1
            reqs.append(r)
    ...
    async def mpi_recv(self, buf, size, _tag):
        ...
        req = MPI.COMM_WORLD.Irecv([buf, MPI.BYTE], source=you, tag=_tag)
